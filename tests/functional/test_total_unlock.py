...
def test_empty(hegicKeep3r):
    assert hegicKeep3r.ethCalculateTotalUnlock([]) == 0
...
def test_simple(hegicKeep3r, mockedHegicOptions):
    ...
    assert hegicKeep3r.ethCalculateTotalUnlock(list(range(3))) == Wei("122.1 ether")
...
def test_with_non_expired(hegicKeep3r, mockedHegicOptions):
    ...
    with brownie.reverts():
        hegicKeep3r.ethCalculateTotalUnlock(list(range(2)))
