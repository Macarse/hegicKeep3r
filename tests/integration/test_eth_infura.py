...
def test_eth(HegicKeep3r, deployer, alice, bob):
    ...
    tx = hegicOptions.create(
        24 * 3600, Wei("1 ether"), 2000 * 1e8, 2, {"from": bob, "value": bob.balance()}
    )
    optionIDs[0] = tx.events["Create"]["id"]

    tx = hegicOptions.create(
        24 * 3600, Wei("2 ether"), 2000 * 1e8, 2, {"from": bob, "value": bob.balance()}
    )
    optionIDs[1] = tx.events["Create"]["id"]
    tx = hegicOptions.create(
        24 * 3600, Wei("10 ether"), 2000 * 1e8, 2, {"from": bob, "value": bob.balance()}
    )
    optionIDs[2] = tx.events["Create"]["id"]

    assert hegicKeep3r.ethOptionsUnlockable(optionIDs) == False
    ...
    assert hegicKeep3r.ethOptionsUnlockable(optionIDs) == True

    # Only keepers can unlock through the keep3r contract
    with brownie.reverts():
        hegicKeep3r.ethUnlockAll(optionIDs, {"from": bob})

    t = hegicKeep3r.ethUnlockAll(optionIDs, {"from": keeper})
